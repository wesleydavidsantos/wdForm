...
abstract class AbstractRelationship implements InterfaceRelationship
{
	...
	public function construct_inner_join_sql(Table $from_table, $using_through=false, $alias=null)
	{
		if ($using_through)
		{
             ...
             * @obs Informa o ALIAS em vez do nome da tabela
            */
            $class_name = Table::load($this->class_name)->className;
            if( isset( $class_name::$alias_table ) )
                $from_table_name = $class_name::$alias_table;
            else
		...
		else
		{
             ...
             * @obs Informa o ALIAS em vez do nome da tabela
            */
            $class_name = $from_table->className;
            if( isset( $class_name::$alias_table ) )
                $from_table_name = $class_name::$alias_table;
            else
